...
def extract_text_impl(pdf_path: str):
    """
    pdfminer.six:
      - extract_text avec LAParams pour améliorer le regroupement ligne/blocs
      - compte pages via PDFPage
    """
    warnings = []
    meta = {"pages": None}
    ...
    try:
        ...
        raise RuntimeError(f"pdfminer import failed: {e}")

    # Compte pages (utile pour tes métriques)
    try:
        with open(pdf_path, "rb") as fp:
            meta["pages"] = sum(1 for _ in PDFPage.get_pages(fp))
    except Exception:
        warnings.append("pdfminer_page_count_failed")

    # Paramètres layout : à tuner, mais ceux-ci marchent bien sur des CV
    laparams = LAParams(
        line_margin=0.2,      # rapprochement vertical des lignes
        word_margin=0.1,      # rapprochement horizontal des mots
        char_margin=2.0,      # tolérance caractères (aide quand spans éclatés)
        boxes_flow=0.5,       # compromis lecture colonnes vs flow
    ...
    try:
        ...
        warnings.append("pdfminer_extract_failed")
        raise RuntimeError(f"pdfminer extract_text failed: {e}")

    if not text.strip():
        warnings.append("pdfminer_empty_output")
